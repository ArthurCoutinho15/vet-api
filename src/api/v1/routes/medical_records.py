...
async def post_medical_record(
    medical_record: MedicalRecordSchema, db: AsyncSession = Depends(get_session)
):
...
async def get_medical_record(
    medical_record_id: int, db: AsyncSession = Depends(get_session)
):
    async with db as session:
...
@router.put(
    "/{id}", status_code=status.HTTP_202_ACCEPTED, response_model=MedicalRecordSchema
)
